...
function App() {
  ...
  // 🔴 NUEVO: Estados para responsividad móvil
  ...
  const selectAccount = async (acc: Account) => {
    ...
    // 🔴 NUEVO: Cerrar sidebar en móvil al seleccionar cuenta
  ...
  return (
    <div className="h-screen flex flex-col font-sans text-white overflow-hidden" style={{ background: '#0D1117' }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* 🔴 NUEVO: Hamburger Menu (solo móvil) */}
        {isMobile && !sidebarOpen && (
          <button
            onClick={() => setSidebarOpen(true)}
            className="fixed top-4 left-4 z-50 p-2 rounded-lg border border-white/[0.1]"
        ...
        `} style={{ background: '#111820' }}>
          ...
          <div className={`flex items-center ${sidebarOpen ? 'justify-between px-4' : 'justify-center'} py-3`}>
            ...
            <div className="flex items-center gap-1">
              ...
              {/* 🔴 MODIFICADO: Botón X en móvil, collapsar en desktop */}
              <button 
                onClick={() => setSidebarOpen(!sidebarOpen)} 
                className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-white/[0.08] transition-colors" 
                title={sidebarOpen ? (isMobile ? 'Close' : 'Collapse') : 'Expand'}
              >
                {isMobile && sidebarOpen ? (
                  <X className="w-3.5 h-3.5 text-white/35" />
                ) : sidebarOpen ? (
                  <ChevronLeft className="w-3.5 h-3.5 text-white/35" />
                ) : (
          ...
          <div className="flex-1 overflow-y-auto px-1.5 space-y-0.5 pb-2">
            ...
            {accounts.length === 0 && sidebarOpen ? (
              ...
              return (
                <button key={acc.id} onClick={() => selectAccount(acc)} title={!sidebarOpen && !isMobile ? acc.company_name : undefined}
                  className={`w-full text-left rounded-lg transition-all group flex items-center ${sidebarOpen || isMobile ? 'px-3 py-2.5 gap-2.5' : 'justify-center py-2 px-0'} ${isActive ? 'bg-white/[0.08]' : 'hover:bg-white/[0.04]'}`}>
                  <div className="relative shrink-0">
                    <div className={`${sidebarOpen || isMobile ? 'w-8 h-8 text-xs' : 'w-9 h-9 text-sm'} rounded-lg flex items-center justify-center font-bold`}
          ...
          <div className="border-t border-white/[0.06] p-2">
            <button onClick={() => { setSelectedAccount(null); setAnalysis(null); setView('home'); }}
              ...
              className={`w-full flex items-center ${sidebarOpen || isMobile ? 'gap-2.5 px-3 py-2.5' : 'justify-center py-2.5'} rounded-lg transition-all ${
              ...
              {(sidebarOpen || isMobile) && <span className={`text-[13px] font-medium ${view === 'home' ? 'text-white/90' : 'text-white/45'}`}>Dashboard</span>}
            </button>
          </div>
          ...
          <div className="border-t border-white/[0.06] p-2">
            <button
              ...
              className={`w-full flex items-center ${sidebarOpen || isMobile ? 'gap-2.5 px-3 py-2.5' : 'justify-center py-2.5'} rounded-lg transition-all hover:bg-white/[0.04] text-white/40 hover:text-white/80`}
            >
              <LogOut className="w-4 h-4 shrink-0" />
              {(sidebarOpen || isMobile) && <span className="text-[13px] font-medium">Cerrar Sesión</span>}
            </button>
          </div>
        </aside>

        {/* 🔴 NUEVO: Backdrop para cerrar sidebar en móvil */}
        {isMobile && sidebarOpen && (
          <div
        ...
        <main className={`flex-1 overflow-y-auto ${isMobile && sidebarOpen ? 'hidden' : 'block'}`} style={{ paddingBottom: view === 'home' ? '20px' : chatOpen ? '340px' : '60px' }}>
          <div className="max-w-5xl mx-auto px-4 md:px-8 py-8">
            ...
            <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 mb-8">
              <div className="relative">
                <button onClick={() => setRoleOpen(!roleOpen)} className="h-12 w-full md:w-auto px-5 rounded-xl flex items-center gap-2.5 text-sm font-semibold transition-colors border border-white/[0.1] hover:border-white/[0.2]" style={{ background: 'rgba(255,255,255,0.05)' }}>
              ...
              <div className="flex-1 flex items-center h-12 rounded-xl border border-white/[0.1] px-5 gap-3 focus-within:border-[#01A982]/50 transition-colors" style={{ background: 'rgba(255,255,255,0.04)' }}>
                ...
                <span className="hidden md:flex text-xs text-white/25 items-center gap-1.5 shrink-0"><Clock className="w-3.5 h-3.5" /> 3 months</span>
              </div>
              <button onClick={handleAnalyze} disabled={loading || !company}
                className="h-12 px-7 rounded-xl font-bold text-base transition-all flex items-center justify-center gap-2.5 disabled:opacity-30 text-white shrink-0 hover:brightness-110"
                style={{ background: loading ? '#333' : '#01A982' }}>
                {loading ? <RefreshCw className="animate-spin w-5 h-5" /> : <ArrowRight className="w-5 h-5" />}
                <span className="hidden md:inline">{loading ? 'Scanning...' : 'Analyze'}</span>
                <span className="md:hidden">{loading ? 'Scanning' : 'Go'}</span>
              </button>
            </div>
              ...
              <div className="space-y-6">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <h1 className="text-2xl md:text-3xl font-black text-white">{selectedAccount.company_name}</h1>
                  ...
                  <div className="space-y-2">
                    {alerts.filter(a => a.account_id === selectedAccount.id).slice(0, 3).map(a => (
                      <div key={a.id} className="flex items-center gap-3 px-5 py-3 rounded-xl border" style={{ borderColor: `${alertLevelColor(a.alert_level)}25`, background: `${alertLevelColor(a.alert_level)}0A` }}>
                        ...
                        <span className="text-[10px] text-white/25 shrink-0 hidden md:block">{fmtTime(a.created_at)}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="rounded-xl p-5 border border-white/[0.06]" style={{ background: 'rgba(255,255,255,0.025)' }}>
                    ...
                    ) : (<div className="space-y-1.5"><p className="text-sm text-white/55">{selectedAccount.industry || <span className="text-white/25 italic">No industry</span>}</p>{selectedAccount.website && <p className="text-sm text-white/40 break-all">{selectedAccount.website}</p>}{selectedAccount.last_product && selectedAccount.last_product !== 'None' && <p className="text-sm mt-2"><span className="text-white/30">Last rec:</span> <span className="font-semibold" style={{ color: '#01A982' }}>{selectedAccount.last_product}</span></p>}</div>)}
                  </div>
                </div>
                ...
                <div className="rounded-xl p-5 border border-white/[0.06]" style={{ background: 'rgba(1,169,130,0.03)' }}>
                  <SectionToggle label="Sales Context" icon={FileText} color="rgba(1,169,130,0.6)" open={showSalesContext} onToggle={() => setShowSalesContext(!showSalesContext)}
                    extra={<span className="text-xs text-white/25 font-normal normal-case tracking-normal hidden md:inline">personalizes speeches</span>} />
                  {showSalesContext && (
                    editingAccount ? (
                      ...
                      <div key={h.id} className="rounded-xl border border-white/[0.06] hover:border-white/[0.1] transition-all mb-3 overflow-hidden" style={{ background: 'rgba(255,255,255,0.025)' }}>
                        <button onClick={() => setExpandedHistory(isExpanded ? null : h.id)} className="w-full text-left px-5 py-3.5 flex items-center justify-between hover:bg-white/[0.03] transition-colors">
                          ...
                          <div className="flex items-center gap-3">
                            {triggers.length > 0 && <span className="text-xs px-2.5 py-1 rounded-full bg-yellow-500/10 text-yellow-500/70 font-medium hidden md:inline">{triggers.length} signals</span>}
              ...
              <div className="space-y-8">
                ...
                <div>
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                    <div>
                      <h1 className="text-2xl md:text-3xl font-black text-white tracking-tight">Priority Accounts</h1>
                  ...
                  {(() => {
                    ...
                    // 🔴 NUEVO: Empty State mejorado para home
                    if (ranked.length === 0) return (
                      <div className="rounded-2xl border border-dashed border-white/[0.1] p-12 md:p-16 text-center">
                        <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center mx-auto mb-5 md:mb-6" style={{ background: 'rgba(1,169,130,0.08)' }}>
                          <Target className="w-8 h-8 md:w-10 md:h-10" style={{ color: '#01A982', opacity: 0.4 }} />
                        </div>
                        <h3 className="text-lg md:text-xl font-bold text-white/40 mb-2 md:mb-3">No Accounts Yet</h3>
                        <p className="text-sm text-white/25 max-w-sm mx-auto mb-6">
                          Start adding accounts to monitor buying signals and get AI-powered sales intelligence.
                        </p>
                        <button
                          onClick={() => setShowAddModal(true)}
                          className="inline-flex items-center gap-2 px-6 py-3 rounded-lg font-semibold text-white hover:brightness-110 transition-all"
                          ...
                          return (
                            <div key={acc.id}
                              ...
                              <div className="flex items-center gap-3 md:gap-5 px-4 md:px-6 py-4 md:py-5">
                                {/* Rank */}
                                <div className={`w-10 h-10 md:w-12 md:h-12 rounded-xl flex items-center justify-center shrink-0 text-base md:text-lg font-black ${
                                  i === 0 ? 'text-white' : ''
                                }`} style={{
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex flex-wrap items-center gap-2 md:gap-3">
                                    <h3 className={`font-bold truncate ${i === 0 ? 'text-lg md:text-xl text-white' : 'text-base md:text-lg text-white/80'}`}>{acc.company_name}</h3>
                                    {accAlerts.length > 0 && (
                                  ...
                                  <div className="flex flex-wrap items-center gap-2 md:gap-3 mt-1">
                                    ...
                                    {acc.contact_name && <span className="text-xs text-white/30 hidden md:inline">{acc.contact_name}{acc.contact_role ? ` · ${acc.contact_role}` : ''}</span>}
                                  </div>
                                  {/* Why Now? — top trigger */}
                                  {topTriggers[acc.id] && topTriggers[acc.id].length > 0 && (() => {
                                    ...
                                    return (
                                      <div className="flex items-center gap-2 mt-2">
                                        <span className="text-[10px] font-bold uppercase tracking-wider text-yellow-500/70 shrink-0 hidden md:inline">Why now?</span>
                                        <div className="flex items-center gap-1.5 min-w-0">
                                          <IC className="w-3 h-3 shrink-0" style={{ color: ac }} />
                                          <span className="text-xs text-white/45 truncate">{t.type?.replace(/_/g, ' ')}: {t.description?.substring(0, 50)}{(t.description?.length || 0) > 50 ? '...' : ''}</span>
                                        </div>
                                        {topTriggers[acc.id].length > 1 && (
                                          <span className="text-[10px] text-white/25 shrink-0">+{topTriggers[acc.id].length - 1}</span>
                                        )}
                                      </div>
                                    );
                                  })()}
                                ...
                                <div className="text-right shrink-0">
                                  <div className="flex items-baseline gap-1 justify-end">
                                    <span className={`font-black ${i === 0 ? 'text-3xl md:text-4xl' : 'text-2xl md:text-3xl'}`} style={{ color: sc }}>{acc.last_score}</span>
                                    <span className="text-xs text-white/20">/100</span>
                                  </div>
                                  <p className="text-[10px] uppercase tracking-[0.15em] font-bold mt-0.5 hidden md:block" style={{ color: sc }}>{lbl}</p>
                                  <div className="w-16 md:w-20 h-1.5 rounded-full bg-white/[0.08] mt-2 ml-auto overflow-hidden">
                                    <div className="h-full rounded-full" style={{ width: `${acc.last_score}%`, background: sc }} />
                                  </div>
                                </div>

                                {/* Arrow */}
                                <ArrowRight className="w-5 h-5 text-white/20 group-hover:text-white/50 transition-colors shrink-0 hidden md:block" />
                              </div>
                            </div>
                          );
                        })}
                ...
                <div className="rounded-2xl border border-white/[0.08] overflow-hidden" style={{ background: 'rgba(255,255,255,0.02)' }}>
                  <div className="px-4 md:px-6 py-4 border-b border-white/[0.08] flex items-center gap-3" style={{ background: 'rgba(22,27,34,0.6)' }}>
                    <Bot className="w-5 h-5" style={{ color: '#01A982' }} />
                    <span className="text-sm font-semibold text-white/60">AI Assistant</span>
                    <span className="text-xs text-white/25 hidden md:inline">— Ask about HPE products, accounts, or sales strategies</span>
                  </div>
                  <div className="h-[280px] overflow-y-auto px-4 md:px-6 py-4 space-y-4">
                      ...
                      <div className="text-center py-8">
                        ...
                        <div className="flex gap-2 justify-center flex-wrap">
                          {['Which HPE products fit hybrid cloud?', 'How to pitch HPE GreenLake?', 'Search latest IT spending trends', 'Compare HPE vs Dell for storage'].map((s, i) => (
                            <button key={i} onClick={() => setHomeChatInput(s)} className="text-xs text-white/35 bg-white/[0.04] px-4 py-2 rounded-full border border-white/[0.07] hover:bg-white/[0.08] transition-colors hidden md:inline-block">{s}</button>
                          ))}
                        </div>
                      </div>
                    )}
                    {homeChatMessages.map((m, i) => (
                      <div key={i} className={`flex gap-3 ${m.role === 'user' ? 'justify-end' : ''}`}>
                        ...
                        <div className={`max-w-[85%] md:max-w-[70%] rounded-xl px-4 py-3 text-sm leading-relaxed ${m.role === 'user' ? 'bg-white/[0.1] text-white/80' : 'text-white/60'}`} style={m.role === 'assistant' ? { background:'rgba(1,169,130,0.05)' } : {}}>
                  ...
                  <div className="px-4 md:px-6 py-3.5 border-t border-white/[0.08]" style={{ background: 'rgba(22,27,34,0.4)' }}>
                    <div className="flex items-center gap-3">
            ...
            {analysis && isCompetitor && (
              <div className="rounded-2xl p-8 md:p-12 text-center border border-red-500/20" style={{ background: 'linear-gradient(135deg, rgba(239,68,68,0.08) 0%, transparent 100%)' }}>
                <ShieldAlert className="w-12 h-12 md:w-14 md:h-14 text-red-500 mx-auto mb-4" /><h3 className="text-lg md:text-xl font-bold text-red-400 mb-3">Direct HPE Competitor</h3>
            ...
            {analysis && !hasOpportunity && !isCompetitor && (
              <div className="rounded-2xl p-8 md:p-12 text-center border border-white/[0.06]" style={{ background: 'rgba(255,255,255,0.025)' }}>
                <XCircle className="w-12 h-12 md:w-14 md:h-14 text-white/20 mx-auto mb-4" /><h3 className="text-lg md:text-xl font-bold text-white/50 mb-3">No Signals Detected</h3>
            ...
            {/* ═══ ANALYSIS RESULTS (continúa igual pero con algunos ajustes responsive) ═══ */}
              ...
              <div className="space-y-8">
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <h1 className="text-3xl md:text-4xl font-black text-white tracking-tight break-words">{analyzedCompany}</h1>
                  ...
                  <div className="text-left md:text-right">
                    <div className="flex items-baseline gap-1 justify-start md:justify-end"><span className="text-4xl md:text-5xl font-black" style={{ color: scoreColor }}>{fitScore}</span><span className="text-lg text-white/25">/100</span></div>
                    <p className="text-xs uppercase tracking-[0.15em] font-bold mt-1" style={{ color: scoreColor }}>{scoreLabel} PRIORITY</p>
                    <div className="w-full md:w-28 h-1.5 rounded-full bg-white/[0.08] mt-2.5 md:ml-auto overflow-hidden"><div className="h-full rounded-full" style={{ width: `${fitScore}%`, background: scoreColor }} /></div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {top3.map((rec, i) => (
                      <div key={i} className="rounded-xl p-4 md:p-5 border transition-colors" style={{ borderColor: i === 0 ? 'rgba(1,169,130,0.2)' : 'rgba(255,255,255,0.06)', background: i === 0 ? 'linear-gradient(135deg, rgba(1,169,130,0.06) 0%, transparent 100%)' : 'rgba(255,255,255,0.025)' }}>
                        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-3">
                          ...
                          <a href={rec.url} target="_blank" rel="noopener noreferrer" className="text-xs px-3 py-1.5 rounded-full border flex items-center justify-center gap-1.5 hover:bg-white/5 shrink-0 font-medium" style={{ color: '#01A982', borderColor: 'rgba(1,169,130,0.25)' }}><ExternalLink className="w-3 h-3" /> HPE.com</a>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-2 mt-3">
                ...
                {/* 🔴 NUEVO: Triggers con Empty State mejorado */}
                <div>
                  <SectionToggle label="Buying Signals" icon={Zap} color="#EAB308" open={showSignals} onToggle={() => setShowSignals(!showSignals)} count={webData.triggers?.length || 0} />
                  {showSignals && (
                    webData.triggers?.length > 0 ? (
                      ...
                      // 🔴 NUEVO: Empty State para triggers
                ...
                {/* Continúa con el resto del análisis igual pero responsive... */}
                {/* 4-Part Speech */}
                <div>
                      ...
                      <div key={key} className={idx < 3 ? 'border-b border-white/[0.06]' : ''}>
                        <div className="px-4 md:px-6 py-5">
                          <div className="flex items-center gap-3 mb-2.5">
                            ...
                            <span className="text-[10px] text-white/25 hidden md:inline">— {desc}</span>
                          </div>
                          <p className="text-base text-white/60 leading-relaxed pl-0 md:pl-9">{(analysis as any)[key] || <span className="text-white/20 italic">Not generated</span>}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <SectionToggle label="Internal Brief" icon={FileText} color="#60A5FA" open={showBrief} onToggle={() => setShowBrief(!showBrief)}
                    extra={<span className="text-xs text-white/25 font-normal normal-case tracking-normal hidden md:inline">HPE Sales Team only</span>} />
                  {showBrief && (
                    <div className="rounded-2xl border overflow-hidden" style={{ borderColor: 'rgba(59,130,246,0.12)', background: 'rgba(59,130,246,0.03)' }}>
                      <div className="px-4 md:px-6 py-3.5 flex items-center justify-end border-b" style={{ borderColor: 'rgba(59,130,246,0.1)' }}>
                      ...
                      <div className="px-4 md:px-6 py-5"><p className="text-base text-white/55 leading-relaxed whitespace-pre-line">{analysis.speech_resumen || 'Not available.'}</p></div>
                    </div>
                  )}
                </div>

                {/* 🔴 NUEVO: Pain Points, Tech, Competitors con Empty States */}
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                      ...
                      <div className="rounded-xl p-5 border border-white/[0.06]" style={{ background: 'rgba(255,255,255,0.02)' }}>
                        ...
                        {webData.tech_stack?.length > 0 ? (
                          <div className="flex flex-wrap gap-2">{webData.tech_stack.map((t, i) => <span key={i} className="px-2.5 py-1 rounded-lg bg-white/[0.06] text-white/50 text-sm border border-white/[0.06]">{t}</span>)}</div>
                        ) : (
                          // 🔴 NUEVO: Empty State para tech stack
                          <div className="text-center py-6">
                            ...
                            <p className="text-white/20 text-xs mt-1">General IT infrastructure recommendations can still apply</p>
                          </div>
                        )}
                      </div>
      ...
      {/* ═══ CHAT DRAWER (hidden on home, con ajuste para móvil) ═══ */}
      {view !== 'home' && <div className={`fixed bottom-0 z-40 transition-all ${sidebarOpen && !isMobile ? 'left-64' : 'left-0'} right-0`} style={{ background: '#161B22' }}>
        <button onClick={() => setChatOpen(!chatOpen)} className="w-full h-12 flex items-center justify-between px-4 md:px-6 border-t border-white/[0.1] hover:bg-white/[0.03] transition-colors">
        ...
        {chatOpen && (
          <div className="border-t border-white/[0.08]" style={{ height: '300px' }}>
            <div className="h-[248px] overflow-y-auto px-4 md:px-6 py-4 space-y-4">
              {chatMessages.length === 0 && (
                <div className="text-center py-10"><Bot className="w-10 h-10 text-white/15 mx-auto mb-3" /><p className="text-sm text-white/30">Ask about accounts, HPE products, or request research.</p>
                  <div className="hidden md:flex gap-2 justify-center mt-4 flex-wrap">
            ...
            <div className="px-4 md:px-6 py-3 border-t border-white/[0.08]">
              <div className="flex items-center gap-3">
                <input type="text" placeholder="Ask anything..." className="flex-1 bg-white/[0.05] border border-white/[0.08] rounded-lg px-4 py-2.5 text-sm text-white placeholder-white/30 outline-none focus:border-[#01A982]/50"
          ...
          <div className="rounded-2xl w-full max-w-xl border border-white/[0.1] shadow-2xl" style={{ background: '#1C2128' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="p-7 space-y-6 max-h-[70vh] overflow-y-auto">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-4 md:gap-6 text-sm text-white/40">
